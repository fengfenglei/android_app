...
public class NoListAdapter extends BaseAdapter {
	private  List<CollectShop.ChoicenessGoods> datas = new ArrayList<>();
	private Context context;

	public NoListAdapter(Context context, List<CollectShop.ChoicenessGoods> datas){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(position * 2 + 1 < datas.size()){
			...
			pkId1 = String.valueOf(datas.get(position * 2).getGoodsId());
			pkId2 = String.valueOf(datas.get(position * 2 + 1).getGoodsId());
...
//			hodler.textView1.setText(item1);
			...
			Glide.with(context).load(Urls.URL_PHOTO+"/file/v3/download-"+icon1+"-300-300.jpg").into(hodler.icon1);
...
//			hodler.textView2.setText(item2);
			...
			Glide.with(context).load(Urls.URL_PHOTO+"/file/v3/download-"+icon2+"-300-300.jpg").into(hodler.icon2);
